...
//% addr.defl=39
namespace lcd16x02 {
export function inite(addr: number): void {
    i2cAddr = addr
    basic.pause(50)
    write(0x33, 0)
    basic.pause(5)
    ...
    write(0x0C, 0)
    write(0x06, 0)
    lcd16x02.clear() // Chamada com prefixo para evitar erro de referência
}

//% block="limpar LCD"
export function clearr(): void {
    write(0x01, 0)
    basic.pause(2)
}

//% block="exibir texto %s na linha %l coluna %c"
//% l.min=1 l.max=2 c.min=0 c.max=15
export function showStrings(s: string, l: number, c: number): void {
    let addr = (l == 2 ? 0x40 : 0) + c
    write(0x80 | addr, 0)
...
/**
   ...
   */
    ...
    namespace calculadora {
        ...
        export function processarr(tecla: string): void {
